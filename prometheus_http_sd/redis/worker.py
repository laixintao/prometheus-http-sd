...
from ..metrics import generator_latency, finished_jobs, worker_jobs_processed, worker_started_counter
...
class Worker:
    ...
    def _process_job(self, job_data: dict):
        ...
        logger.info(f"Worker {self.worker_id} processing job {job_id} for path: {path}")
        start_time = time.time()
        
        try:
            # Track generator latency
            with generator_latency.labels(full_path=full_path, status="success").time():
            ...
            if self.cache.set(full_path, cache_data, config.cache_expire_seconds):
                duration = time.time() - start_time
                logger.info(f"Worker {self.worker_id} completed job {job_id} in {duration:.2f}s")
                finished_jobs.inc()
                worker_jobs_processed.labels(worker_id=self.worker_id, status="success").inc()
            else:
                logger.error(f"Worker {self.worker_id} failed to cache results for job {job_id}")
                worker_jobs_processed.labels(worker_id=self.worker_id, status="cache_error").inc()
            
        except Exception as e:
            with generator_latency.labels(full_path=full_path, status="error").time():
                pass
            ...
            logger.error(f"Worker {self.worker_id} failed to process job {job_id} for path '{path}': {e}")
            ...
            logger.debug(f"Cached error details for {full_path} under key {error_cache_key}")
            
            worker_jobs_processed.labels(worker_id=self.worker_id, status="error").inc()
        finally:
            try:
                success = self.queue.complete_job(job_data)
                if success:
                    logger.debug(f"Worker {self.worker_id} successfully completed job {job_id}")
                else:
                    logger.warning(f"Worker {self.worker_id} failed to complete job {job_id} in queue")
            except Exception as e:
                logger.error(f"Worker {self.worker_id} error completing job {job_id}: {e}")

class WorkerPool:    
    def __init__(self, num_workers: int = 4, first_worker_id: str = None, metrics_port: int = 8081):
