...
class RedisJobQueue:    
    def __init__(self, redis_url: str = "redis://localhost:6379/0", queue_name: str = "target_generation_queue"):
    ...
    def is_job_queued_or_processing(self, full_path: str) -> bool:
        """Check if a job for the given full_path is already queued or being processed."""
        try:
            if not self._connected:
                self._connect()
            ...
            logger.debug(f"Checking {main_queue_count} jobs in main queue for {full_path}")
            ...
            processing_jobs = self._redis_client.lrange(self.processing_queue_name, 0, -1)
            processing_count = len(processing_jobs)
            logger.debug(f"Checking {processing_count} jobs in processing queue for {full_path}")
    ...
    def enqueue_job(self, job_data: Dict[str, Any]) -> bool:
        try:
            ...
            result = self._redis_client.lpush(self.queue_name, json.dumps(job_data))
    ...
    def complete_job(self, job_data: Dict[str, Any]) -> bool:
        try:
            ...
            # Get all jobs in processing queue and find the one with matching job_id
            processing_jobs = self._redis_client.lrange(self.processing_queue_name, 0, -1)
            for job_json in processing_jobs:
                try:
                    job = json.loads(job_json)
                    if job.get('job_id') == job_id:
                        ...
                        result = self._redis_client.lrem(self.processing_queue_name, 1, job_json)
